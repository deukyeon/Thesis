...
used lock-based protocol. It operates in two distinct phases:
...
\emph{no-wait}~\cite{DBLP:conf/sigmod/RenFA16,DBLP:conf/usenix/TangE18}, where
transactions immediately abort if they cannot acquire a needed lock, rather than
waiting and potentially creating deadlocks.
...
operations per second (IOPS)~\cite{samsung2013ssd}. However, HDDs are still used
in many systems because of their low cost and high capacity.
...
single-digit microseconds and throughput in the tens of
GB/s~\cite{yang2023overcoming}. CXL-based SSDs are still in the early stages of
development, but they are already being used in some
systems~\cite{samsung2022fms}.
\begin{itemize}
...
\textbf{Log-Structured Merge (LSM) trees}~\cite{OneillChGaOn96} and similar
...
random queries (reads). For example, SplinterDB~\cite{splinterdb}, which uses a
write-optimized structure called a \bets~\cite{BrodalFa03}, can perform over 2
million random insertions per second but only about 500,000 random queries per
second.
...
To provide concrete, data-driven motivation, we highlight our benchmark results
that illustrate both the potential and the challenges of timestamp-based
concurrency control in on-disk systems.
