...
mechanisms~\cite{yu2016tictoc,wolf2015sto,tu13silo,plor,polaris,2PLFS,chardonnay,bamboo}.
...
timestamps of keys accessed by both transactions. In TicToc, a transaction's
commit timestamp must be within the range of [write timestamp ($wts$), read
timestamp ($rts$)] of the version that it read and must be greater than $rts$ of
the previous version it wrote. This ability to decouple physical commit order
from logical timestamp order ultimately reduces conflicts and improves
concurrency.
...
consider a transaction that reads a key whose true timestamps are $wts=100$ and
...
any timestamp in the range $[100, 160]$ when using precise timestamps (since the
commit timestamp must be within [$wts$, $rts$] of the version it read), the
overapproximation reduces this viable range to $[150, 160]$, eliminating 50
