...
(MVTO)~\cite{reed1983mvto}, and newer types like TicToc~\cite{yu2016tictoc}.
...
For example, in a basic STO system~\cite{bernstein1987concurrency}, just reading
a data item means also writing its read timestamp back to disk. Updating a row
also requires reading its old timestamps before doing anything. This extra
metadata overhead turns simple, fast operations into slower ones that involve
both reading and writing, reducing the system's performance.
...
timestamp-based CC methods including TicToc~\cite{yu2016tictoc} dynamically
choose a transaction timestamp during the validation phase based on the
timestamps of the keys being accessed, rather than simply assigning a timestamp
at the beginning of the transaction. In both cases, timestamps cannot be
discarded simply because no transaction is currently accessing a key, as future
transactions will need this information to make correct concurrency control
decisions. However, maintaining exact timestamps for all keys in memory is
prohibitively expensive for large databases, creating the very storage overhead
that undermines timestamp-based protocols.
...
sufficient to maintain only an upper bound on timestamps for inactive keys.
Timestamp-based CC protocols compare transaction timestamps against key
timestamps to make abort decisions. If inactive keys' timestamps were to be
completely forgotten, the critical information needed to enforce serializability
when new transactions access these keys would be lost. However, by maintaining
an overapproximation (upper bound) instead of exact values, correctness is
preserved while dramatically reducing memory requirements. This
overapproximation may cause harmless extra transaction aborts, but it will not
violate database correctness.


\section{\sketchname: A New Approach for Timestamp-Based CC on Disk}

The first main contribution presents a new approach for efficient
...
protocol itself (such as STO~\cite{bernstein1987concurrency} or
...
While \sketchname works well on modern NVMe SSDs, this result raises a deeper
research question: Is using approximate, in-memory metadata always useful, or is
its benefit only clear for fast storage devices like NVMe SSDs? Separating the
concurrency control logic from storage performance seems powerful, but this
requires evaluation across all types of storage, from older, slower devices to
the most modern memory-like storage.
...
First, consider using \sketchname in HDDs and SATA SSDs. In these
