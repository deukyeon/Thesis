...
timestamps of keys accessed by both transactions. This ability to decouple
physical commit order from logical timestamp order ultimately reduces conflicts
...
consider a transaction that accesses a key whose true timestamp is 100 but has
been overapproximated to 150. TicToc must assume the key was last modified at
time 150, forcing it to choose a commit timestamp of at least 151. If the
transaction could have committed with any timestamp in the range 101-160 when
using precise timestamps, the overapproximation reduces this viable range to
151-160, eliminating 50 possible commit timestamps.
