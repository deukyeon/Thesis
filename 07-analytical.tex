...
latencies~\cite{samsung2013ssd} to emerging byte-addressable storage like
CXL-based flash approaching DRAM-like speeds~\cite{yang2023overcoming}. Since
\sketchname enables modern timestamp-based concurrency control for disk-based
key-value stores, evaluating its effectiveness across this performance spectrum
is critical for understanding both current deployment opportunities and future
applicability as storage technologies continue to evolve.
...
\begin{itemize}
    \item \sketchname variants achieve significant performance improvements across
    both SATA SSD and HDD, with up to 569\% and 519\% higher goodput than Disk
    variants in write-intensive, high-contention workloads on SATA SSD and HDD,
    respectively. TicToc-\psketchname outperforms 2PL and KR-OCC by up to 589\% and
    152\% on SATA SSD, and achieves 1.8$\times$ faster than KR-OCC on HDD.
...
\begin{itemize}
    \item On fast storage (emulated DRAM-like latency storage), \sketchname
...
we emulate CXL-based flash storage using block
ramdisk~\cite{linuxkernel_ramdisk}. This subsection details our emulation
