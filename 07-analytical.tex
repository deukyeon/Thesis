Storage media span from the next-generation low-latency byte-addressable fabric
such as CXL-based flash with DRAM-like latency to traditional high-capacity but
high-latency HDDs. As \sketchname enables the modern concurrency-controls to
disk-based key-value stores, it is important to evaluate \sketchname on various
storage media. Because production systems use various storage media, we believe
evaluating \sketchname in various storage media scenarios will be crucial. It
not only uncovers opportunities for adoption in wider range of systems but also
can provide a guidance for future development as storage media continue to be faster.
We evaluate \sketchname in both slow and fast storage media scenarios.
