...
and HDD) in Section~\ref{sec:slow-storage} and fast storage (simulated CXL-based
SSDs) in Section~\ref{sec:fast-storage} to reveal how \sketchname's benefits
and trade-offs evolve with storage characteristics. Our analysis demonstrates
that \sketchname remains effective across this wide range, though the nature of
its advantages changes fundamentally as storage performance improves.
...
different machine because CloudLab does not offer the same machines with HDDs.
Using SATA SSD allows us to compare results fairly with NVMe SSD and other
faster storage. The HDD experiments demonstrate how \sketchname performs on a
real HDD disk, helping us understand its behavior in truly slow storage
situations.

This section evaluates \sketchname on slow storage media, revealing the following:
\begin{itemize}
    \item \sketchname variants achieve significant performance improvements
    across both SATA SSD and HDD, with up to 569\% and 519\% higher goodput than
    Disk variants in write-intensive, high-contention workloads on SATA SSD and
    HDD, respectively. TicToc-\psketchname outperforms 2PL and KR-OCC by up to
    6.89$\times$ and 2.52$\times$ on SATA SSD, and achieves 1.8$\times$ faster
    than KR-OCC on HDD.
    
    \item \sketchname demonstrates effectiveness across diverse workload types,
    with up to 60\% higher goodput than Disk variants and up to 24\% and 59\% over
    2PL and KR-OCC in mixed-transaction workloads. For medium-contention workloads,
    \sketchname improves goodput by up to 19\% and 121\% for read-intensive and
    write-intensive workloads on HDD. In TPC-C with 4 warehouses,
    STO-\psketchname and TicToc-\psketchname achieve up to 1.25$\times$ and
    1.18$\times$ better goodput than 2PL/KR-OCC on HDD, demonstrating consistent
    benefits across synthetic and realistic benchmarks.
\end{itemize}
...
particular is 1.65$\times$ and 1.19$\times$ faster than 2PL and KR-OCC,
respectively, in workloads with high contention.
...
results, delivering 6.89$\times$ and 2.52$\times$ higher goodput than 2PL and
KR-OCC, respectively, for high-contention workloads.
...
It also outperforms 2PL and KR-OCC, with goodput increases of up to 1.24$\times$ and
1.59$\times$, respectively.
...
its performance on fast storage media, such as CXL-based SSDs with DRAM-like 
...
\begin{itemize}
    ...
    control. TicToc-\psketchname achieves up to 3.55$\times$ and 21.5$\times$
    higher goodput than 2PL and KR-OCC in high-contention workloads, with up to
    286\% improvement over TicToc-Disk. STO-\psketchname achieves up to
    11.9--70.6$\times$ more goodput than KR-OCC (and up to 1.41$\times$ more
    than 2PL) in TPC-C, demonstrating that timestamp-based methods excel when
    storage latency is no longer the bottleneck.
    
    \item Performance improvements extend consistently across synthetic and
    realistic benchmarks. In write-intensive workloads, TicToc-\psketchname is
    2.12$\times$ and 3.16$\times$ faster than 2PL and KR-OCC, with 200\% improvement
    over TicToc-Disk. For mixed-transaction workloads, TicToc-\psketchname achieves
    1.78$\times$ and 24.3$\times$ higher throughput than 2PL and KR-OCC, with 77\%
    improvement over TicToc-Disk. TPC-C results show 34--45\% and 107--150\%
    improvements over Disk variants for TicToc and STO, respectively, across all
    contention levels.
...
we simulate CXL-based SSDs storage using block
ramdisk~\cite{linuxkernel_ramdisk}. This subsection details our simulation
methodology, the assumptions we make, and how our results relate to real CXL
hardware.

\subsubsection{Simulation Setup}

We use block ramdisk on the same CloudLab machine described in
...
This approach allows us to evaluate \sketchname's behavior when storage access
...
Our simulation makes several important assumptions that should be considered
...
\begin{itemize}
    ...
    characteristics we seek to simulate are preserved.
    
    \item \textbf{Bandwidth characteristics}: Real CXL storage provides high
    bandwidth (up to 16 GB/s on PCIe 5.0 x4), whereas our simulation is limited
    by system RAM bandwidth. However, since our evaluation focuses on
    latency-bound operations (random I/O for timestamp access), bandwidth
    differences have minimal impact on our findings.
    
    \item \textbf{Remote memory effects}: Real CXL storage may exhibit NUMA-like
    behavior with remote memory access characteristics. Our simulation does not
...
methods as we change the number of warehouses (which also changes the contention
...
than TicToc for long transactions. This matches our earlier findings from the
...
storage simulating CXL-based SSDs with DRAM-like latency. Our experimental study
...
high-contention workloads. On fast storage (CXL-based SSDs or similar future
innovations), \sketchname remains beneficial, particularly for high-contention
scenarios where timestamp-based methods significantly outperform 2PL and KR-OCC.
However, the overhead becomes more visible, suggesting future optimization
opportunities.
