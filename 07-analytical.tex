...
\begin{itemize}
    \item On fast storage (emulated CXL with latencies 100$\times$ faster than SATA
    SSD and 10$\times$ faster than NVMe SSD), \sketchname demonstrates dramatic
    performance advantages over traditional concurrency control. TicToc-\psketchname
    achieves up to 3.55$\times$ and 21.5$\times$ higher goodput than 2PL and KR-OCC
    in high-contention workloads, with up to 286\% improvement over TicToc-Disk.
    STO-\psketchname achieves up to 11.9--70.6$\times$ more goodput than 2PL/KR-OCC
    in TPC-C, demonstrating that timestamp-based methods excel when storage latency
    ...
    improvement over TicToc-Disk. TPC-C results show 25--44.5\% and 105--150\%
