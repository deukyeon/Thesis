...
in performance---from traditional hard disk drives (HDDs) with the order of
millisecond latencies~\cite{samsung2013ssd} to emerging byte-addressable storage
like CXL-based SSDs approaching DRAM-like speeds~\cite{yang2023overcoming}.
Since \sketchname enables modern timestamp-based concurrency control for
disk-based key-value stores, evaluating its effectiveness across this
performance spectrum is critical for understanding both current deployment
opportunities and future applicability as storage technologies continue to
evolve.

This chapter presents a comprehensive evaluation across slow storage (SATA SSD
and HDD) in \Cref{sec:slow-storage} and fast storage (simulated CXL-based
SSDs) in \Cref{sec:fast-storage} to reveal how \sketchname's benefits and
trade-offs evolve with storage characteristics. The analytical evaluation
demonstrates that \sketchname remains effective across this wide range, though
the nature of its advantages changes fundamentally as storage performance
improves.


\section{Slow Storage}\label{sec:slow-storage}

Slow storage represents traditional disk-based systems where storage latency
significantly impacts transaction processing performance. On slow storage, disk
I/O becomes the dominant bottleneck, making the elimination of timestamp disk
accesses provided by \sketchname particularly valuable. The results demonstrate
how \sketchname performs under challenging storage conditions and reveal its
effectiveness when storage characteristics fundamentally limit transaction
throughput.

On slow storage media, the traditional CC methods, 2PL and KR-OCC, are the most
relevant baselines because they do not depend on on-disk timestamp metadata;
this avoids extra random I/O overhead on the critical path. Accordingly,
\sketchname is primarily compared against 2PL and KR-OCC, and Disk/Disk-Cache
timestamp variants are used as secondary references to quantify the penalty of
persisting timestamps on slow devices.

The same workloads and evaluation methods as in \Cref{sec:eval} are used.
For slow storage setups, both SATA SSD and HDD are used. The SATA SSD
experiments are conducted on the same type of machine as the NVMe SSD tests,
with only the storage hardware changed. However, for the HDD tests, different
machines are used because CloudLab does not offer the same machines with HDDs.
Using SATA SSD allows comparing results fairly with NVMe SSD and other faster
storage. The HDD experiments demonstrate how \sketchname variants perform on a
real HDD disk, helping understand its behavior in truly slow storage situations.

This section evaluates \sketchname on slow storage media, highlighting the
following:
\begin{itemize}
    \item \Sketchname enables timestamp-based CC to outperform 2PL and KR-OCC in
    high-contention workloads. On SATA SSD, TicToc-\psketchname is up to
    6.89$\times$ and 2.52$\times$ faster than 2PL and KR-OCC, respectively; on
    HDD it achieves 1.8$\times$ the goodput of KR-OCC. Relative to Disk
    variants, \sketchname removes costly timestamp reads, reaching up to 569\%
    (SATA) and 519\% (HDD) higher goodput in write-intensive cases.

    \item Across mixed and long-transaction workloads on slow storage,
    \sketchname continues to exceed 2PL/KR-OCC. In mixed YCSB, it delivers up to
    1.24$\times$ and 1.59$\times$ better goodput than 2PL and KR-OCC,
    respectively, while also improving up to 60\% over Disk variants by
    eliminating timestamp I/O. Under medium contention on HDD, where I/O
    dominates, \sketchname still lifts goodput by up to 19\% (read-intensive)
    and 121\% (write-intensive) versus Disk. 

    \item In TPC-C with 4 warehouses, STO-\psketchname and TicToc-\psketchname
    reach up to 1.25$\times$ and 1.18$\times$ better goodput than 2PL/KR-OCC on
    HDD, showing benefits across synthetic and realistic benchmarks.
\end{itemize}
...
\begin{figure}[!t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            ...
            \node[below] at (cc1 |- current bounding box.south) {(a) High-contention YCSB (zipf=0.99)};
            \node[above] at (cc2 |- current bounding box.south) {(b) Medium-contention YCSB (zipf=0.6)};
        \end{tikzpicture}
    ...
    1 write. Note that, in the medium-contention workload, several variants have
    abort rates very close to 0.}
    \label{fig:ycsb:slow_ssd}
\end{figure}

\Cref{fig:ycsb:slow_ssd} presents the goodput of all timestamp-based concurrency
control variants on four YCSB small-transaction workloads using slow SSD
storage. The patterns in the results are similar to those shown in
\Cref{sec:eval} for NVMe SSD. The \sketchname variants regularly achieve goodput
very close to the Memory setup, which shows that they remain effective even when
storage is slower.

In read-intensive workloads, the \sketchname variants (\fsketchname and
\psketchname) outperform the slow-storage baselines: TicToc-\psketchname is
1.65$\times$ and 1.19$\times$ faster than 2PL and KR-OCC, respectively, under
high contention, while almost matching the Memory setup. They also improve
goodput by as much as 39\% compared to Disk variants when contention is high,
and up to 23\% in medium-contention situations.

For write-intensive workloads, the difference in goodput becomes even greater.
The \sketchname variants again stay close to Memory and clearly exceed 2PL and
KR-OCC: under high contention, TicToc-\psketchname delivers 6.89$\times$ and
2.52$\times$ higher goodput than 2PL and KR-OCC, respectively. Relative to Disk
variants, they boost goodput by up to 569\% in high contention and 119\% in
medium contention.

TicToc is a timestamp-based CC protocol designed for high concurrency. Paired
with \sketchname, it delivers the best performance across all workloads because
\sketchname eliminates many on-disk timestamp lookups, reducing storage-induced
stalls. The benefit is most pronounced in write-intensive scenarios:
SplinterDB's fast write path allows operations to proceed quickly even with slow
storage, but serializing transactions requires reading timestamps from disk,
turning writes into reads. \sketchname effectively mitigates this overhead.
...
\begin{figure}[!t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            ...
            \node[above] at (c |- current bounding box.north) {\ref{grouplegend}};
            \node[below] at (cc1 |- current bounding box.south) {(a) High-contention (zipf=0.99)};
            \node[above] at (cc2 |- current bounding box.south) {(b) Medium-contention (zipf=0.6)};
        \end{tikzpicture}
    ...
    perform 28 reads. Note that, in the medium-contention workload, several
...
scenarios, \sketchname outperforms 2PL and KR-OCC by up to 1.24$\times$ and
1.59$\times$, respectively. It also offers up to 60\% higher goodput than the
Disk variants by avoiding timestamp I/O.

Under medium contention, the goodputs of these systems are mainly limited by
slow disk access. Unlike the evaluation with NVMe SSDs described in
...
long-transaction workload on slow SSD. The main pattern mirrors prior sections:
the \sketchname variants maintain higher goodput than 2PL/KR-OCC and their Disk
versions. Because the disk is slow, long transactions run more slowly and the
differences between methods are smaller than with NVMe SSD
(\Cref{fig:ycsb:long}). Even so, \sketchname continues to deliver the best
performance among all tested protocols.
...
\begin{figure*}[!t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            ...
            \node[above] at (c |- current bounding box.north) {\ref{grouplegend}};
        \end{tikzpicture}
    } \caption[TPC-C results on slow SSD]{TPC-C results with 120 processing
    threads on slow SSD (More warehouses means less contention).}
    \label{fig:tpcc:slow_ssd}
\end{figure*}

\Cref{fig:tpcc:slow_ssd} shows the goodput for the same TPC-C workloads
described in \Cref{sec:eval-setup}. Compared to 2PL, \sketchname variants
achieve better goodput in high-contention cases (4 and 8 warehouses) by avoiding
on-disk timestamp metadata. At lower contention, 2PL can be competitive with
TicToc-\sketchname on slow disks due to abort patterns: 2PL more often aborts
shorter \texttt{Payment} transactions, whereas TicToc tends to abort longer
\texttt{NewOrder} transactions. For example, in the 32-warehouse scenario, 93\%
of aborts in 2PL are \texttt{Payment}, while 79\% in TicToc are
\texttt{NewOrder}. Re-executing longer transactions harms TicToc goodput on slow
disks, making TicToc-\psketchname 0.89$\times$ as fast as 2PL at 32 warehouses,
but 1.5$\times$ faster at 4 warehouses. Relative to Disk variants, \sketchname
consistently removes timestamp I/O and improves goodput.

Having examined SATA SSD performance, the evaluation now turns to HDDs to evaluate
\sketchname under even more challenging storage conditions. HDDs represent the
slowest storage type in this evaluation, with latencies measured in milliseconds
rather than microseconds. This allows observing how \sketchname's effectiveness
scales as storage becomes increasingly I/O-bound and understanding its behavior
in truly slow storage environments where disk access dominates all other
factors.
...
For these experiments, a different machine is used since CloudLab does not
provide the same machine with an HDD. The system used for the HDD evaluation
...
tests, 60 threads are used because the version of SplinterDB used requires
...
\begin{figure}[!t]
    ...
    Note that, in the medium-contention workload, several variants have abort
...
TicToc-\psketchname has the highest goodput in this workload, running 1.04$\times$
...
performance. TicToc-\psketchname achieves 1.8$\times$ the speed of KR-OCC and
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            \begin{groupplot}[group style={group size=4 by 1,
                ...
                % graph [2,1] abort rate
                \nextgroupplot[AbortRateBarChartPlot, ylabel={Abort rate}, ylabel style={yshift=-3pt}, ymax=0.25]
...
\Cref{fig:tpcc:hdd} presents the goodput results for the TPC-C workloads
...
Having demonstrated \sketchname's effectiveness on slow storage, the evaluation
now examines its performance on fast storage media, such as CXL-based SSDs with
DRAM-like latency. The evaluation reveals a fundamental shift in performance
...
This section presents \sketchname's performance on fast storage, revealing the following:
\begin{itemize}
    \item On fast storage (simulated DRAM-like latency storage), \sketchname
    demonstrates dramatic performance advantages over traditional concurrency
    control across all workload types. In high-contention, read-intensive
    workloads, TicToc-\psketchname achieves up to 3.55$\times$ and 21.5$\times$
    higher goodput than 2PL and KR-OCC, respectively, with up to 286\%
    improvement over TicToc-Disk. In write-intensive workloads,
    TicToc-\psketchname is 2.12$\times$ and 3.16$\times$ faster than 2PL and
    KR-OCC, with 200\% improvement over TicToc-Disk. For mixed-transaction
    workloads, TicToc-\psketchname achieves 1.78$\times$ and 24.3$\times$ higher
    throughput than 2PL and KR-OCC, with 77\% improvement over TicToc-Disk.
    TPC-C results show consistent improvements: TicToc-\psketchname achieves
    34--45\% higher goodput than TicToc-Disk across all contention levels, while
    STO-\psketchname achieves up to 11.9--70.6$\times$ more goodput than KR-OCC
    (and up to 1.41$\times$ more than 2PL) with 107--150\% improvement over
    STO-Disk, demonstrating that timestamp-based methods excel when storage
    latency is no longer the bottleneck.
    
    \item As storage becomes faster, \sketchname variants exhibit overhead from
    hash table operations, memory allocation, and sketch eviction that create a
    performance gap compared to the idealized Memory variant (39.5\% and 22.5\%
    lower for TicToc-\psketchname and STO-\psketchname, respectively). CPU cycles
    become the limiting factor rather than storage latency, revealing opportunities
    for future optimization, particularly for medium-contention workloads where
    overheads are more visible.
\end{itemize}

\subsection{CXL Simulation Methodology}

To evaluate \sketchname's effectiveness on next-generation storage technologies,
CXL-based SSDs storage is simulated using block
ramdisk~\cite{linuxkernel_ramdisk}. This subsection details the simulation
methodology, the assumptions made, and how these results relate to real CXL
hardware.

\subsubsection{Simulation Setup}

Block ramdisk is used on the same CloudLab machine described in
\Cref{sec:eval-setup} to simulate CXL-based SSDs with DRAM-like latency. Block
ramdisk provides storage access through the standard block device interface
while storing data in RAM, effectively creating a software-based storage layer
that mimics the low latency characteristics of memory-like storage technologies.
This approach allows evaluating \sketchname's behavior when storage access
times are dramatically reduced without requiring specialized CXL hardware that
may not be readily available in experimental environments.


\subsubsection{Assumptions and Limitations}

The simulation makes several important assumptions that should be considered
when interpreting results:

\begin{itemize}
    \item \textbf{Interface semantics}: Block ramdisk uses the standard block
    device interface, whereas real CXL storage may support byte-addressable
    access through the CXL.mem protocol. This difference may affect the relative
    performance of different access patterns, though the fundamental latency
    characteristics the simulation seeks to capture are preserved.
    
    \item \textbf{Bandwidth characteristics}: Real CXL storage provides high
    bandwidth (up to 16 GB/s on PCIe 5.0 x4), whereas the simulation is limited
    by system RAM bandwidth. However, since the evaluation focuses on
    latency-bound operations (random I/O for timestamp access), bandwidth
    differences have minimal impact on the findings.
    
    \item \textbf{Remote memory effects}: Real CXL storage may exhibit NUMA-like
    behavior with remote memory access characteristics. The simulation does not
    capture these effects, though \sketchname's effectiveness in avoiding remote
    access overhead remains valid.
    
    \item \textbf{CPU overhead}: Block ramdisk still incurs some software
    overhead from the block device stack, whereas real CXL storage with direct
    memory access may have different overhead characteristics. However, this
    overhead is minimal compared to actual storage latency and does not
    materially affect the conclusions.
\end{itemize}

\subsubsection{Experimental Configuration}

In these fast storage experiments, only STO and TicToc protocols are evaluated.
...
evaluation on fast storage where write rates are extremely high. 120 processing
threads are run for all experiments, matching the configuration used in the NVMe
SSD evaluations. All other experimental parameters (workloads, database size,
...
\begin{figure}[!t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            \begin{groupplot}[group style={group size=4 by 2,
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 100.589433) (3.1, 342.33)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 32.581733) (3.1, 53.617733)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 34.518667) (3.1, 60.1803)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 52.053633) (3.1, 105.591)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 78.006133) (3.1, 206.962)};
                \coordinate (top) at (rel axis cs:0,1);% coordinate at top of the first plot
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 20.868633) (3.1, 48.751833)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 11.414233) (3.1, 13.5591)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 10.320467) (3.1, 12.3741)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 17.8436) (3.1, 30.488667)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 19.657167) (3.1, 40.6375)};

                % graph [1,1] medium
                \nextgroupplot[title={Read-intensive},YCSBThroughputBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 272.525667)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 5.632817)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 304.898) (3.1, 305.503333)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 189.604) (3.1, 113.674333)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 71.498267) (3.1, 71.392467)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 149.862667) (3.1, 151.15)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 235.684667) (3.1, 235.993333)};
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 293.822) (3.1, 293.970333)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 154.005) (3.1, 146.303)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 70.924433) (3.1, 71.719567)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 156.197) (3.1, 161.700667)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 255.064) (3.1, 255.141333)};
                \coordinate (bot) at (rel axis cs:1,0);% coordinate at bottom of the last plot
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.314407) (3.1, 0.130448)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.300011) (3.1, 0.136726)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.296994) (3.1, 0.130361)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.386568) (3.1, 0.256559)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.330712) (3.1, 0.149822)};

                % graph [1,4] write intensive
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.513741)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.493239)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.517988) (3.1, 0.28678)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.516791) (3.1, 0.27266)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.535364) (3.1, 0.292757)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.538097) (3.1, 0.337119)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.517645) (3.1, 0.297033)};

                % graph [1,2]
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.000413)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.000167)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.000085) (3.1, 0.000031)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.000081) (3.1, 0.000045)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.000098) (3.1, 0.000032)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.001611) (3.1, 0.000105)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.000211) (3.1, 0.00008)};

                % graph [1,4]
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.002108)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.001008)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.000498) (3.1, 0.000191)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.000456) (3.1, 0.000237)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.000605) (3.1, 0.000398)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.007669) (3.1, 0.000779)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.003707) (3.1, 0.000566)};
    ...
    and 1 write. Note that, in the medium-contention workload, several variants
    have abort rates very close to 0.}
    \label{fig:ycsb:fast_storage}
\end{figure}

\Cref{fig:ycsb:fast_storage} presents the goodput results for all
...
effective. Although Disk variants store timestamps on SplinterDB with fast
...
become significant. The overhead has several components: (1) Hash table
operations: \sketchname must create and delete entries in a hash table, which
requires allocating and copying memory for keys based on their reference count.
In the high-contention, read-intensive workload for TicToc-\psketchname, roughly
80\% of \texttt{IncRef} calls are responsible for creating new entries in the
hash table, which increases overhead. This dynamic memory allocation for hash
table entries and key storage incurs CPU cycles and potential cache misses. (2)
Sketch eviction: \fsketchname has additional overhead because it needs to evict
keys from the hash table to the sketch when reference counts reach zero, a step
that does not occur in the \psketchname variant. These overheads collectively
create a performance gap between Memory and \sketchname variants that grows
larger on fast storage, where CPU cycles become the limiting factor rather than
storage latency.
...
\begin{figure}[!t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            \begin{groupplot}[group style={group size=4 by 1,
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 180.33) (3.1, 311.536)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 92.825067) (3.1, 125.022333)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 64.094933) (3.1, 91.446867)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 71.368433) (3.1, 105.553667)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 140.384333) (3.1, 221.610333)};
                \coordinate (top) at (rel axis cs:0,1);% coordinate at top of the first plot
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.126144) (3.1, 0.058634)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.122566) (3.1, 0.057911)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.136498) (3.1, 0.059566)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.239943) (3.1, 0.117789)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.139826) (3.1, 0.065083)};

                % graph [3,1] medium
                \nextgroupplot[YCSBThroughputBarChartPlot, ylabel={Goodput (KTPS)}, ylabel style={yshift=-8pt}]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 556.961)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 7.140063)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 720.277667) (3.1, 714.753333)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 345.637667) (3.1, 267.104333)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 135.666333) (3.1, 134.658333)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 68.007267) (3.1, 284.942667)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 315.562667) (3.1, 475.911667)};

                % graph [4,1] medium abort rate
                \nextgroupplot[AbortRateBarChartPlot, ylabel={Abort rate}, ylabel style={yshift=-3pt}]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.000335)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.000148)};
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.000432) (3.1, 0.000202)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.000239) (3.1, 0.000221)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.000194) (3.1, 0.000078)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.257643) (3.1, 0.00023)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.08147) (3.1, 0.000191)};
                \coordinate (bot) at (rel axis cs:1,0);% coordinate at bottom of the last plot
    ...
    transactions perform 28 reads. Note that, in the medium-contention workload,
...
similar to those observed in the small-transaction workloads, but the
...
For the medium-contention scenario, there are two main points. First, similar to
...
For example, increasing the sketch size for STO-\psketchname from the current
32KiB to 256KiB raises goodput to approximately 520 KTPS, improving goodput by
50.4\% compared to STO-Disk. Thus, the more memory for sketch is necessary for
STO if transactions are short and their operations are executed quickly.
    ...
    \resizebox{.75\textwidth}{!}{%
        \begin{tikzpicture}
            \begin{groupplot}[group style={
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 14.4813) (3.1, 9.571037)};
                \addplot[Disk-BarStyle] coordinates {(1.05, 13.3011) (3.1, 9.311203)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 12.304867) (3.1, 8.088823)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 13.978767) (3.1, 9.499523)};
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 14.869067) (3.1, 9.124267)};
                \coordinate (top) at (rel axis cs:0,1);% coordinate at top of the first plot
                ...
                \addplot[Memory-BarStyle] coordinates {(1.05, 0.260036) (3.1, 0.24408)}; % double check
                \addplot[Disk-BarStyle] coordinates {(1.05, 0.29484) (3.1, 0.248819)}; % double check sto
                \addplot[Disk-Cache-BarStyle] coordinates {(1.05, 0.307315) (3.1, 0.256368)}; % double check sto
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.05, 0.267667) (3.1, 0.246097)}; % double check
                \addplot[FPSketch-BarStyle] coordinates {(1.05, 0.263106) (3.1, 0.247183)}; % double check
                \coordinate (bot) at (rel axis cs:1,0);% coordinate at bottom of the last plot
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            \begin{groupplot}[group style={group size=4 by 2,
                ...
                % graph [1,1]
                \nextgroupplot[title={4 warehouses},YCSBThroughputBarChartPlot, ylabel={Goodput (KTPS)}]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 40.3478)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 4.766923)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 59.3193) (3, 54.249267)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 23.122733) (3, 29.668767)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 17.227167) (3, 19.149033)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 41.3587) (3, 27.051667)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 56.8103) (3, 42.8906)};
                \coordinate (top) at (rel axis cs:0,1);% coordinate at top of the first plot

                % graph [1,2]
                \nextgroupplot[title={8 warehouses}, YCSBThroughputBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 60.317733)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 2.690287)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 95.317467) (3, 85.340067)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 33.973233) (3, 41.828467)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 29.014767) (3, 25.245733)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 42.335667) (3, 30.594867)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 85.059467) (3, 58.450233)};

                % graph [1,3]
                \nextgroupplot[title={16 warehouses},YCSBThroughputBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 90.617967)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 1.714923)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 126.781667) (3, 113.534667)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 49.8068) (3, 62.967367)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 33.078067) (3, 31.654267)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 37.753067) (3, 38.951133)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 111.767) (3, 78.6518)};


                % graph [1,4] 32wh
                \nextgroupplot[title={32 warehouses},YCSBThroughputBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 118.859)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 1.768823)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 136.254) (3, 128.511333)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 60.4086) (3, 77.2502)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 35.204067) (3, 35.5132)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 54.0724) (3, 62.5415)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 124.853) (3, 103.150333)};

                \coordinate (bot) at (rel axis cs:1,0);% coordinate at bottom of the last plot

                % graph [2,1] 4 wh
                \nextgroupplot[AbortRateBarChartPlot, ylabel={Abort rate}]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.265972)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.582189)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 0.308396) (3, 0.358287)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 0.300735) (3, 0.362429)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 0.306576) (3, 0.373073)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 0.378139) (3, 0.439741)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 0.316455) (3, 0.38327)};

                % graph [2,2] 8 wh
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.197449)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.619373)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 0.259365) (3, 0.328457)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 0.249634) (3, 0.322022)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 0.259498) (3, 0.320605)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 0.383305) (3, 0.448231)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 0.279779) (3, 0.364156)};

                % graph [2,3] 16 wh
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.130951)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.626907)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 0.194619) (3, 0.282973)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 0.189055) (3, 0.271532)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 0.180874) (3, 0.280659)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 0.572401) (3, 0.40728)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 0.23819) (3, 0.320579)};

                % graph [2,4] 32 wh
                \nextgroupplot[AbortRateBarChartPlot]
                \addplot[2PL-BarStyle, forget plot] coordinates {(-0.5, 0.107609)};
                \addplot[KR-OCC-BarStyle, forget plot] coordinates {(-0.26, 0.512981)};
                \addplot[Memory-BarStyle] coordinates {(1.06875, 0.040332) (3, 0.202382)};
                \addplot[Disk-BarStyle] coordinates {(1.06875, 0.067348) (3, 0.215892)};
                \addplot[Disk-Cache-BarStyle] coordinates {(1.06875, 0.063018) (3, 0.20933)};
                \addplot[Counter-Lazy-BarStyle] coordinates {(1.06875, 0.74949) (3, 0.291205)};
                \addplot[FPSketch-BarStyle] coordinates {(1.06875, 0.167312) (3, 0.247521)};
            \end{groupplot}
            \coordinate (c) at ($(top)!.5!(bot)$);
...
methods as the number of warehouses changes (which also changes the contention
level). Similar to the YCSB results, both \psketchname and \fsketchname always
have lower goodput than the Memory version because of extra overheads, but they
are still much better than the Disk-based methods.

TicToc-\psketchname achieves up to 9 to 58.3$\times$ higher goodput than KR-OCC
(though it can be slower than 2PL at lower contention levels with
0.87--1.06$\times$ relative performance), and about 34--45\% higher goodput
than TicToc-Disk at all contention levels. STO-\psketchname achieves up to 11.9
to 70.6$\times$ more goodput than KR-OCC and up to 1.41$\times$ more than 2PL,
and about 107--150\% improvement over STO-Disk in all cases. The TPC-C benchmark
consists of both short and long transactions, and STO achieves higher goodput
than TicToc for long transactions. This matches earlier findings from the
YCSB long-transaction workload in \Cref{fig:ycsb:long_fast}.

Comparing \psketchname and \fsketchname, \psketchname always has
better goodput at every contention level. This is because \psketchname has no
overhead from sketch evictions. This indicates that \psketchname is a good
option for deployment on fast storage.
...
storage simulating CXL-based SSDs with DRAM-like latency. The experimental study
reveals several important findings and lessons that guide the adoption and
future development of \sketchname.

\subsection{Key Findings}

The evaluation demonstrates that \sketchname remains effective across a wide
range of storage technologies. On slow storage media, including SATA SSD and
HDD, \sketchname enables timestamp-based protocols to outperform traditional
concurrency control methods. In high-contention, write-intensive workloads on
SATA SSD, TicToc-\psketchname achieves up to 6.89$\times$ and 2.52$\times$
higher goodput than two-phase locking (2PL) and KR-OCC, respectively, while in
read-intensive workloads it reaches 1.65$\times$ and 1.19$\times$ faster than
2PL and KR-OCC. On HDD, TicToc-\psketchname achieves up to 1.8$\times$ the
goodput of KR-OCC in write-intensive workloads, and in TPC-C with 4 warehouses,
STO-\psketchname and TicToc-\psketchname reach up to 1.25$\times$ and
1.18$\times$ better goodput than 2PL/KR-OCC. Also, \sketchname continues to
provide substantial improvements over traditional methods by eliminating the
overhead of accessing timestamps from disk.

On fast storage media, the experiments reveal a different set of
...
The experimental study yields several important lessons. First, the
effectiveness of \sketchname varies with storage characteristics, and the
appropriate baselines vary accordingly. On slow storage (I/O-bound), 2PL and
KR-OCC are the right baselines because they avoid on-disk timestamp metadata; in
this regime, \sketchname enables timestamp-based CC to exceed those baselines by
eliminating timestamp I/O. On fast storage (CPU-bound), while \sketchname still
outperforms Disk-based approaches and surpasses 2PL/KR-OCC by scaling
concurrency, the relative overhead of the \sketchname data structure becomes
more visible. This suggests future optimizations should focus on reducing CPU
overheads, particularly for high-performance storage environments.

Second, \psketchname consistently outperforms \fsketchname
...
Third, these results highlight the importance of transaction characteristics in
...
Fourth, sketch capacity and adaptivity matter as storage becomes faster.
Performance is sensitive to the size of the sketch, especially for STO on fast
storage where immediate validation increases pressure on approximate timestamps.
Larger sketches reduce over-approximation and aborts (e.g., increasing the
sketch from 32KiB to 256KiB substantially improved STO-\psketchname in mixed
workloads). A simple autotuning policy that adjusts sketch size using online
abort/eviction telemetry within a small memory budget can deliver robust
performance across workloads.

Finally, CPU and NUMA considerations dominate on fast storage. Once the system
is CPU-bound, allocator and hash table costs in \sketchname become critical.
Using slab/arena allocation for entries and keys, pre-sizing and per-core/socket
sharding of the hash table, reducing key copies, and NUMA-aware placement can
close part of the gap to the Memory configuration.


\subsection{Deployment Guidelines}

Based on this comprehensive evaluation, the following deployment
guidelines for \sketchname are offered:
...
\sketchname provides dramatic improvements against traditional concurrency
control methods and should be adopted whenever timestamp-based CC methods are
used. The benefits are most pronounced for high-contention workloads. On fast
storage (CXL-based SSDs or similar future innovations), \sketchname remains
beneficial, particularly for high-contention scenarios where timestamp-based
methods significantly outperform 2PL and KR-OCC. However, the overhead becomes
more visible, suggesting future optimization opportunities.

\textbf{Workload considerations:} \sketchname is highly beneficial for the vast
majority of practical workloads. Small transactions with high contention see the
biggest wins; Long transactions also benefit, though proportionally less because
transaction execution time dominates. Mixed workloads benefit substantially from
\sketchname across all storage types.

\textbf{Memory requirements:} \sketchname requires minimal memory (32KiB in the
experiments with an 80GB database), making it practical for deployment even in
memory-constrained environments. The memory footprint remains consistent across
storage types, as the sketch size is workload-dependent rather than
storage-dependent.
